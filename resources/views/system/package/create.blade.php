                                ...
                                <div  class="col-md-12 param-div">
                                    ...
                                    @php
                                        ...
                                        if(isset($_GET['service_id']) && !empty($_GET['service_id'])){
                                          $parent_data =  getService($_GET['service_id']);
                                          if($parent_data)
                                          $servicesData[$parent_data->id] = $parent_data->{'title_'.lang()};
                                          }
                                        foreach ($services as $key => $value){
                                            $servicesData[$value->id] = $value->name;
                                        }
                                    @endphp
                                    {!! Form::select('service_id',$servicesData,isset($result) ? $result->parent_id: null,['style'=>'width: 100%','class'=>'form-control services-select','id'=>'service_id-form-input','autocomplete'=>'off']) !!}
                        ...
                        <div class="tab-pane" id="k_portlet_base_demo_2_3_tab_content" role="tabpanel">
                            ...
                            <div class="form-group row">
                                <div class="col-md-6">
                                    ...
                                    {!! Form::text('discount_from',(isset($result) && !empty($result->discount_from) && $result->discount_from!= '0000-00-00') ? date('Y-m-d',strtotime($result->discount_from)) : null,['class'=>'form-control k_datepicker_1','id'=>'discount_from-form-input','autocomplete'=>'off']) !!}
                                    <div class="invalid-feedback" id="discount_from-form-error"></div>
                                </div>
                                <div class="col-md-6">
                                    <label>{{__('Discount To')}}</label>
                                    {!! Form::text('discount_to',(isset($result) && !empty($result->discount_to) && $result->discount_to!= '0000-00-00') ? date('Y-m-d',strtotime($result->discount_to)) : null,['class'=>'form-control k_datepicker_1','id'=>'discount_to-form-input','autocomplete'=>'off']) !!}
                            ...
                            <div class="form-group row">
                                <div class="col-md-6">
                                    ...
                                    {!! Form::text('discount_code_from',(isset($result) && !empty($result->discount_code_from) && $result->discount_code_from != '0000-00-00') ? date('Y-m-d',strtotime($result->discount_code_from)) : null,['class'=>'form-control k_datepicker_1','id'=>'discount_code_from-form-input','autocomplete'=>'off']) !!}
                                    <div class="invalid-feedback" id="discount_code_from-form-error"></div>
                                </div>
                                <div class="col-md-6">
                                    <label>{{__('Discount Code To')}}</label>
                                    {!! Form::text('discount_code_to',(isset($result) && !empty($result->discount_code_to) && $result->discount_code_to != '0000-00-00') ? date('Y-m-d',strtotime($result->discount_code_to)) : null,['class'=>'form-control k_datepicker_1','id'=>'discount_code_to-form-input','autocomplete'=>'off']) !!}
...
@section('footer')
    ...
    <script type="text/javascript">
        ...
        function submitMainForm(){
            formSubmit(
                '{{isset($result) ? route('system.package.update',$result->id):route('system.package.store')}}',
                $('#main-form').serialize(),
                function ($data) {
        ...
        $(document).ready(function() {
            ...
            $('#images-form-input').fileuploader({
                ...
                upload: {
                    url: '{{route('system.package.image-upload')}}',
                    data: {
                        '_token': '{{csrf_token()}}',
                        'key':  '{{$randKey}}',
                ...
                onRemove: function(item) {
                    ...
                    $.post('{{route('system.package.remove-image')}}', {
                        'name': imageName,
                        '_token': '{{csrf_token()}}',
                        'key':  randKey
                    });
